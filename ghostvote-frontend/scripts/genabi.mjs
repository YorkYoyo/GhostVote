...
const HARDHAT_DEPLOYMENTS_ROOT = path.resolve(ROOT, "ghostvote-hardhat", "deployments");
...
function readDeployment(network) {
  ...
  const file = path.resolve(dir, "GhostVote.json");
...
function main() {
  ...
  if (!localhost && !sepolia) {
    console.log("GhostVote not deployed on localhost or sepolia. Skip genabi.");
    return;
  }

  // write ABI - prefer localhost ABI if exists, else sepolia
  const gg = localhost || sepolia;
  const abiTs = `export const GhostVoteABI = {\n  abi: ${JSON.stringify(gg.abi, null, 2)}\n};\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "GhostVoteABI.ts"), abiTs);
  ...
  const addrTs = `export const GhostVoteAddresses = ${JSON.stringify(addresses, null, 2)} as const;\n`;
  fs.writeFileSync(path.resolve(FRONTEND_ABI_DIR, "GhostVoteAddresses.ts"), addrTs);
